...
**Execution:** The training program ran without sub-agents. The user explicitly authorized parallel sub-agents for the September 13 GitHub upload and WireGuard route. Start each step as soon as its prerequisites are satisfied. There are no fixed implementation slots or per-step durations. Downloads and training processes may run while Codex continues independent work.

**Status, 13 September 2026:** All thirteen component categories now have separate models that pass their declared signal-description tests. Selected checkpoints and evaluation exports are verified. The dated radiator-valve model failed twice. A separate industrial-valve sound model passed under the documented unknown-date exception. All three owned VMs are stopped. Estimated total spend is $111.35. These results do not establish failure prediction. The HDD model completed all 11,315,069 training windows. All 93 tests pass in working and isolated locked environments, including native TimeF checks. The operator demo remains separate and unfinished. See [component results](component-model-results.md) and [the component program](component-training-program.md). The shared ceiling is $600. The user authorized the full GitHub upload on September 13, 2026.
...
**Dataset scope confirmed:** complete the historical Backblaze HDD model first. If it passes, the user now authorizes research and separate TSLMs for the other hardware components in the [dataset backlog](dataset-backlog.md), within a total $600 compute ceiling. Prefer separate models and data; combine only when the available data cannot support separation. Follow the [component training program](component-training-program.md). Document work in this GitHub repository. The September 13 upload request supersedes the earlier commit and push prohibition.
